...
app.get("/:id", (req, res) => {
    ...
    // === : valeur et type sont égaux
    const contact = contacts.find(contact => contact.id === id);
    // on créé un objet javascript contact - syntaxe raccourcie identique à ceette syntaxe : res.json({
    //     contact: contact
    // });
    res.json({contact});
});
...
// On créé et démarre le serveur et on lui dit d’écouter sur le port défini
// Le serveur transmettra ensuite les requêtes à l'application Express qui cherchera comment y répondre grâce à ses routes.
